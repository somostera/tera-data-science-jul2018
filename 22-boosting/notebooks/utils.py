...
colors = seaborn.color_palette('Set1')
colors_str = ','.join(colors.as_hex())
...
def load_cancer():
	...
	return x, y, list(cancer.target_names)
...
def load_heights():
    ...
    return a[:,0].reshape(-1,1), a[:,1]

def load_fraud():
	frauds = pd.read_csv('../data/fraud.csv')
	y = frauds.is_fraud 
	x = frauds.drop('is_fraud', axis=1)
	return x, y
...
def load_nonlinear():
	return load_pickle('../data/non-linear.p')

def load_playing():
	return load_pickle('../data/playing.p')

def load_regression():
	return load_pickle('../data/regression.p')
...
datasets = {'non-linear': load_nonlinear,
			'playing': load_playing,
			'cancer': load_cancer,
			'fraud': load_fraud,
...
def plot2d(x, y, clf=None):
	if clf is not None:
		...
		plt.scatter(x=x[y==0,0], y=x[y==0,1], color=colors[0])
		plt.scatter(x=x[y==1,0], y=x[y==1,1], color=colors[1])
